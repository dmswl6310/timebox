...
- 블록 이동, 길이 조절, 버퍼 추가, 완료 및 실제 수행 시간 자동 기록
- 타임블록 안의 삭제 버튼으로 일정만 바로 제거하거나, 브레인덤프 작업 삭제 시 연결된 타임블록까지 함께 정리
- 계획 확정 뒤 추가·이동·크기 조정·삭제 시 변경 이유 입력, `일과 완료` 시점의 최종 일정 차이와 이유 저장
- 일기와 기분 자동 저장
...
- 반대로 브레인덤프에서 배치된 작업의 예상 시간을 바꾸면 연결 블록 크기도 동기화합니다. 확정 후에는 이유가 필요하고 다른 블록과 겹치는 변경은 적용하지 않습니다.
...
- 계획 확정 후 일정의 추가·이동·크기 조정·삭제에는 이유가 필요합니다. 중간 편집은 기록하지 않고, `일과 완료`를 누르면 확정본과 최종본을 비교해 최종 추가·이동·크기 변경·삭제와 각 변경 유형의 마지막 이유만 `schedule_change_events`에 기록한 뒤 그날을 잠급니다.
